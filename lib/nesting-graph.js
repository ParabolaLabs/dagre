...
function dfs(g, root, nodeSep, weight, height, depths, v) {
  ...
  let swapDims = (graphLabel.rankdir || "").toLowerCase() === "lr" ||
                 (graphLabel.rankdir || "").toLowerCase() === "rl";
  if (label._iPadTop !== undefined) {
    let dim = Math.max(0, label._iPadTop - graphLabel.ranksep);
    let topNode = g.node(top);
    if (swapDims) { topNode.width = dim; } else { topNode.height = dim; }
  }
  if (label._iPadBottom !== undefined) {
    let dim = Math.max(0, label._iPadBottom - graphLabel.ranksep);
    let bottomNode = g.node(bottom);
    if (swapDims) { bottomNode.width = dim; } else { bottomNode.height = dim; }
  }
