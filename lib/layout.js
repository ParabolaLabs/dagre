...
let graphDefaults = { ranksep: 50, edgesep: 20, nodesep: 50, rankdir: "tb", rankalign: "center", crossrankalign: "center" };
let graphAttrs = ["acyclicer", "ranker", "rankdir", "align", "rankalign", "crossrankalign"];
...
function buildLayoutGraph(inputGraph) {
  ...
  inputGraph.nodes().forEach(v => {
    ...
    if (inputGraph.children(v).length) {
      ...
      let map;
      switch (rd) {
        case "tb": map = [pt, pb, pl, pr]; break;
        case "bt": map = [pb, pt, pl, pr]; break;
        case "lr": map = [pl, pr, pt, pb]; break;
        case "rl": map = [pr, pl, pt, pb]; break;
        default:   map = [pt, pb, pl, pr]; break;
      }
      let keys = ["_iPadTop", "_iPadBottom", "_iPadLeft", "_iPadRight"];
      keys.forEach((k, i) => { if (!isNaN(map[i])) newNode[k] = map[i]; });
    }
  });
